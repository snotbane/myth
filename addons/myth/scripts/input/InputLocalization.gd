...
static func _static_init() -> void:
	...
	refresh_inst()
	ProjectSettings.settings_changed.connect(refresh_inst)


static func refresh_inst() -> void:
	var new_path: String = ProjectSettings.get_setting(PROJECT_SETTING_HINT[&"name"], "")
	if not new_path.is_empty() and new_path == inst_path: return

	inst = load(new_path) if ResourceLoader.exists(new_path) else InputLocalization.new()
	inst_path = new_path if ResourceLoader.exists(new_path) else ""

	if inst == null:
		printerr("WARNING: Attempted to use '%s' for input localization, but it is not a valid path or InputLocalization.")


static func get_platform_name() -> StringName:
	return &"_"
...
static func apply_to(button, event: InputEvent = button.event_staged) -> bool:
	if inst:
...
@export var escape_inputs: Array[InputEvent] = [
	ESCAPE_ESCAPE_KEY.duplicate(),
	ESCAPE_START_KEY.duplicate(),
]
...
@export var show_event_type_icon: bool = true
...
@export var joypad_button_icon_type: Texture2D = preload("res://addons/myth/icons/InputEventJoypadButton.svg")
...
@export var joypad_motion_icon_type: Texture2D = preload("res://addons/myth/icons/InputEventJoypadMotion.svg")
...
@export var key_icon_type: Texture2D = preload("res://addons/myth/icons/InputEventKey.svg")
@export var key_icons: Dictionary[StringName, Array] = {}
@export var key_labels: Dictionary[StringName, PackedStringArray] = {}


@export var mouse_button_icon_type: Texture2D = preload("res://addons/myth/icons/InputEventMouseButton.svg")
...
@export var mouse_motion_icon_type: Texture2D = preload("res://addons/myth/icons/InputEventMouseMotion.svg")
@export var mouse_motion_icon: Texture2D = null
...
func _apply_to(button, event: InputEvent = button.event_staged) -> void:
	if button.button_pressed and button.event_staged == null:
		button.icon = rebind_icon
		button.text = rebind_label if button.icon == null else EMPTY_SPACE_TEXT
		button.tooltip_text = ""

	elif event == null:
		button.icon = unbound_icon
		button.text = unbound_label if button.icon == null else "???"
		button.tooltip_text = unbound_tooltip_text

	elif event is InputEventKey:
		button.icon = get_translation(event.keycode, key_icons)
		...
		if button.icon == null and show_event_type_icon:
			button.icon = key_icon_type
		...
		if button.icon == null and show_event_type_icon:
			button.icon = joypad_button_icon_type
		...
		if button.icon == null and show_event_type_icon:
			button.icon = joypad_motion_icon_type
		...
		if button.icon == null and show_event_type_icon:
			button.icon = mouse_button_icon_type

	elif event is InputEventMouseMotion:
		button.icon = mouse_motion_icon_type if show_event_type_icon else mouse_motion_icon
...
func get_translation(idx: int, category: Dictionary, fallback_value: Variant = null, __platform__: StringName = get_platform_name()) -> Variant:
